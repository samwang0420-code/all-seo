#!/usr/bin/env python3
"""城市站 Place Schema + GeoCoordinates + TouristDestination 补丁"""
import json, re
...
CITY_COORDS = {
    ...
    "wichita": (37.6872,-97.3301), "cleveland": (41.4993,-81.6944),
    "bakersfield": (35.3733,-119.0187), "stockton": (37.9577,-121.2908),
    "corpus-christi": (27.8006,-97.3964), "anchorage": (61.2181,-149.9003),
    "henderson": (36.0395,-114.9817), "garland": (32.9126,-96.6389),
    "reno": (39.5296,-119.8138), "glendale": (33.5387,-112.1859),
    ...
    "jersey-city": (40.7178,-74.0431), "fayetteville": (35.0527,-78.8784),
    "st-petersburg": (27.7676,-82.6403), "laredo": (27.5306,-99.4805),
    "boise": (43.6150,-116.2023), "birmingham": (33.5207,-86.8025),
    "norfolk": (36.8508,-76.2859), "san-bernardino": (34.1083,-117.2898),
    "spokane": (47.6588,-117.4260), "baton-rouge": (30.4515,-91.1871),
    "modesto": (37.6391,-120.9969), "des-moines": (41.5868,-93.6250),
    "port-st-lucie": (27.2730,-80.3582), "huntsville": (34.7304,-86.5861),
    "chattanooga": (35.0456,-85.3097), "tempe": (33.4255,-111.9400),
    "latino": (29.4494,-98.4897),  # san antonio area
    "los-angeles-city": (34.0522,-118.2437), "new-york-city": (40.7128,-74.0060),
}

STATE_POSTAL = {v: k for k, v in {
    ...
    "dc":"DC","district-of-columbia":"DC","puerto-rico":"PR","guam":"GU","virgin-islands":"VI",
}.items()}
...
def slug_to_coords(slug: str) -> tuple:
    ...
    # strip state suffix and try exact match on city name
    for state in STATE_COORDS:
        for sep in ['-', '_']:
            prefix = slug_dash[:-len(sep+state)] if slug_dash.endswith(sep+state) else None
            if prefix and prefix in CITY_COORDS:
                return CITY_COORDS[prefix]
    # state suffix fallback
    for state, sc in STATE_COORDS.items():
        if slug_dash.endswith("-" + state.lower()) or slug_dash.endswith("_" + state.lower()):
            return sc
    return STATE_COORDS["KS"]

def get_state(fp: Path) -> str:
    """从路径提取州代码"""
    parts = fp.parts  # e.g. [..., 'city-data', 'ca', 'arlington-ca.html']
    if len(parts) >= 2:
        st = parts[-2].lower()
        if st in STATE_COORDS:
            return st.upper()
        # lookup state name → postal
        if st in STATE_POSTAL:
            return STATE_POSTAL[st]
    return "CA"
...
def extract_city_name(slug: str) -> str:
    ...
    # Remove state suffix
    for state in STATE_COORDS:
        for suf in [f" {state}", f"-{state.lower()}", f"_{state.lower()}"]:
...
def patch_file(fp: Path):
    ...
    if '"@type": "Place"' in html or '"@type":"Place"' in html:
        if '"@type": "TouristDestination"' in html or '"@type":"TouristDestination"' in html:
            return False  # already has both

    slug = fp.stem  # e.g. "arlington-ca"
    ...
    # Build Place + GeoCoordinates schemas
    place_schema = {
        "@context": "https://schema.org",
        "@type": "Place",
        "name": city_name + ", " + state.upper(),
        "address": {
            "@type": "PostalAddress",
    ...
    tourist_schema = {
        ...
        "name": city_name + ", " + state.upper(),
        "description": f"Travel guide and local information for {city_name}, {state.upper()}. Discover attractions, cost of living, demographics, and visitor resources.",
        "geo": {
            "@type": "GeoCoordinates",
            "latitude": lat,
            "longitude": lng
    ...
    place_block = '\n  <script type="application/ld+json">\n' + json.dumps(place_schema, separators=(',', ':')) + '\n  </script>\n  '
    tourist_block = '\n  <script type="application/ld+json">\n' + json.dumps(tourist_schema, separators=(',', ':')) + '\n  </script>\n  '

    # Inject before the FAQPage schema (first <script type="application/ld+json"> that contains FAQPage)
    ...
    html = html[:script_start] + place_block + tourist_block + html[script_start:]
    fp.write_text(html)
    return True
    ...
    for fp in files:
        try:
            if patch_file(fp):
                ...
                err += 1
                print(f"  WARN: {fp.relative_to(DIR)}: FAQPage not found")
        except Exception as e:
            err += 1
            print(f"  ERROR {fp.relative_to(DIR)}: {e}")
    print(f"✅ Done: {ok} patched, {err} errors")
