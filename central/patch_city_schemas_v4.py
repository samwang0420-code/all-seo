#!/usr/bin/env python3
"""城市站 Place+GeoCoordinates+TouristDestination 补丁 v4 - 精确移除"""
import json, re
...
def remove_old_place_schemas(html: str) -> str:
    """Remove ALL script blocks whose JSON contains @type Place or TouristDestination"""
    # Split HTML by script tag boundaries
    # Strategy: find all <script type="application/ld+json">...</script> blocks
    # and rebuild html without Place/TouristDestination ones
    result = []
    pos = 0
    while True:
        ...
        # Check if this is a Place or TouristDestination schema
        try:
            json_start = script_block.index('{')
            json_text = script_block[json_start:end_tag]
            # Fix trailing commas
            json_text = re.sub(r',(\s*[}\]])', r'\1', json_text)
            data = json.loads(json_text)
            obj_type = data.get('@type', '')
            if obj_type not in ('Place', 'TouristDestination'):
                result.append(script_block)
        except (json.JSONDecodeError, ValueError):
            # Couldn't parse, keep it
            result.append(script_block)
        pos = end_tag + 9
    return ''.join(result)
...
def patch_file(fp: Path) -> bool:
    ...
    # Step 1: remove all existing Place and TouristDestination schemas
    html = remove_old_place_schemas(html)

    # Step 2: build new schemas
    injection = build_injection(city_name, state, lat, lng)

    # Step 3: find FAQPage schema and inject before it
