#!/usr/bin/env python3
"""城市站 Place+GeoCoordinates+TouristDestination 补丁 v3"""
import json, re, html
...
CITY_COORDS = {
    ...
    "arlington-ca":(33.8031,-118.0817),
    "arlington-27-tx":(32.7357,-97.1081),
    "arlington-tx":(32.7357,-97.1081),
    "arlington-va":(38.8786,-77.1043),
    "glendale-az":(33.5387,-112.1859),
    "fayetteville-nc":(35.0527,-78.8784),
    "arlington":(38.8786,-77.1043),  # Default to VA for "arlington" alone
}
...
def parse_json_block(script_content: str):
    """Parse a JSON-LD script content, handling JSON with trailing commas"""
    text = script_content.strip()
    # Remove trailing commas before } or ]
    text = re.sub(r',(\s*[}\]])', r'\1', text)
    return json.loads(text)

def remove_old_schemas(html: str) -> str:
    """Remove existing Place and TouristDestination schemas by parsing JSON blocks"""
    result = html
    # Find all script blocks
    pattern = r'<script type="application/ld\+json">\s*</script>\s*'
    # We need to find actual blocks with content
    while True:
        match = re.search(
        ...
        try:
            data = parse_json_block(json_str)
            obj_type = data.get('@type', '')
            if obj_type in ('Place', 'TouristDestination'):
                result = result[:match.start()] + result[match.end():]
                continue
        except json.JSONDecodeError:
            pass
        # No match or not a target type: advance past this block
        result = result  # keep
        break
    return result
...
def patch_file(fp: Path) -> bool:
    ...
    # Remove any existing Place/TouristDestination schema blocks
    html = remove_old_schemas(html)

    injection = build_injection(city_name, state, lat, lng)

    # Inject before FAQPage schema
...
def main():
    ...
    print(f"✅ Done: {ok} patched, {skip} skipped, {err} errors")
