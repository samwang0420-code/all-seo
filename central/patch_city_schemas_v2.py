#!/usr/bin/env python3
"""城市站 Place+GeoCoordinates+TouristDestination 补丁 v2 - 修正版"""
import json, re
...
CITY_COORDS = {
    ...
    "new-orleans":(29.9511,-90.0715),"arlington-27-tx":(32.7357,-97.1081),
    "arlington-tx":(32.7357,-97.1081),"arlington-va":(38.8786,-77.1043),
    "arlington-ca":(33.8031,-118.0817),  # Arlington CA (Orange County)
    ...
    "reno":(39.5296,-119.8138),"glendale-az":(33.5387,-112.1859),
    ...
    "jersey-city":(40.7178,-74.0431),"fayetteville-nc":(35.0527,-78.8784),
...
def slug_to_coords(slug: str) -> tuple:
    ...
    # strip state suffix then exact match
...
def remove_existing_place_schemas(html: str) -> str:
    """Remove already-injected Place and TouristDestination schema blocks"""
    # Pattern: <script type="application/ld+json"> ... "@type": "Place" ... </script>
    # We remove blocks that contain @type: Place or @type: TouristDestination
    pattern = r'\s*<script type="application/ld\+json">\s*\{[^}]*?"@type":\s*"(?:Place|TouristDestination)"[^}]*\}</script>\s*'
    return re.sub(pattern, '', html, flags=re.DOTALL)

def make_schemas(fp: Path, city_name: str, state: str, lat: float, lng: float) -> str:
    place = {
...
def patch_file(fp: Path):
    ...
    # Remove any existing Place/TouristDestination schemas
    html = remove_existing_place_schemas(html)

    # Build injection
    injection = make_schemas(fp, city_name, state, lat, lng)

    # Find FAQPage schema and inject before it
